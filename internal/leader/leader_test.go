...
func TestMCPServers(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "planner gets base servers",
			role:  RolePlanner,
			extra: nil,
			want:  []string{"notion", "graphiti", "context7"},
		},
		{
			name:  "reviewer gets base servers",
			role:  RoleReviewer,
			extra: nil,
			want:  []string{"notion", "graphiti", "context7"},
		},
		{
			name:  "groomer gets base servers",
			role:  RoleGroomer,
			extra: nil,
			want:  []string{"notion", "graphiti", "context7"},
		},
		{
			name:  "merge gets base plus sequential-thinking",
			role:  RoleMerge,
			extra: nil,
			want:  []string{"notion", "graphiti", "context7", "sequential-thinking"},
		},
		{
			name:  "deployment gets base plus sequential-thinking",
			role:  RoleDeployment,
			extra: nil,
			want:  []string{"notion", "graphiti", "context7", "sequential-thinking"},
		},
		{
			...
			want:  []string{"notion", "graphiti", "context7", "github", "jira"},
		},
		{
			name:  "merge with extra servers",
			role:  RoleMerge,
			extra: []string{"github"},
			want:  []string{"notion", "graphiti", "context7", "sequential-thinking", "github"},
		},
		{
...
func TestPrintBanner(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			databaseID: "ghi789",
			wantIcon:   "🔀",
			wantName:   "Merge Leader",
			wantContains: []string{
				"🔀  Starting Merge Leader session...",
				"Notion Database: ghi789",
			},
		},
		{
			role:       RoleDeployment,
			databaseID: "jkl012",
			wantIcon:   "🚀",
			wantName:   "Deployment Leader",
			wantContains: []string{
				"🚀  Starting Deployment Leader session...",
				"Notion Database: jkl012",
			},
		},
		{
			role:       RoleGroomer,
			databaseID: "mno345",
			wantIcon:   "🧹",
			wantName:   "Backlog Groomer",
			wantContains: []string{
				"🧹  Starting Backlog Groomer session...",
				"Notion Database: mno345",
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(string(tt.role), func(t *testing.T) {
			...
			printBanner(tt.role, tt.databaseID)
			...
			for _, want := range tt.wantContains {
				if !bytes.Contains([]byte(output), []byte(want)) {
					t.Errorf("printBanner(%q, %q) output missing %q\ngot: %q",
						tt.role, tt.databaseID, want, output)
				}
			}
		})
	}
