...
func WorkingGuidelines(w *Worker) string {
	...
	sb.WriteString("### Incremental Development\n\n")
	sb.WriteString("Use draft PRs for early visibility and smaller review chunks:\n\n")
	...
	sb.WriteString("   ```\n\n")
	sb.WriteString("2. **Regular commits**: Commit after each logical chunk\n")
	sb.WriteString("   - Tests passing for that chunk\n")
	sb.WriteString("   - Push every 10-15 minutes of work\n\n")
	sb.WriteString("3. **Mark ready**: When complete, mark PR ready for review\n")
	sb.WriteString("   ```bash\n")
	sb.WriteString("   gh pr ready\n")
	sb.WriteString("   ```\n\n")
